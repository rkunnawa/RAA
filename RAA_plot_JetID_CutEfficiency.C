...
void RAA_plot_JetID_CutEfficiency(){
  ...
  TH1F * hPP_MC_Numerator = (TH1F*)fIn_PP->Get(Form("hpp_JetComb_gen_R%d_n20_eta_p20",radius)); 

  TH1F * hPP_Data_Denominator = (TH1F*)fIn_PP->Get("hpp_Data_Comb_noCut");
  TH1F * hPP_Data_Numerator = (TH1F*)fIn_PP->Get(Form("hpp_HLTComb_R%d_n20_eta_p20",radius)); 
  ...
  for(int i = 0; i<nbins_cent; ++i){
    ...
    hMC_Numerator[i] = (TH1F*)fIn->Get(Form("hpbpb_gen_R%d_n20_eta_p20_cent%d",radius, i));
    hData_Denominator[i] = (TH1F*)fIn->Get(Form("hpbpb_Data_Comb_noCut_cent%d",i));
    hData_Numerator[i] = (TH1F*)fIn->Get(Form("hpbpb_HLTComb_R%d_n20_eta_p20_cent%d",radius,i));
  ...
  cCutEff->SaveAs(Form("../../Plots/%s_Combined_CutEfficiency_centralityClass_R0p%d_%d.pdf",Coll,radius,date.GetDate()),"RECREATE");
  ...
  cCutEff_Data->SaveAs(Form("../../Plots/%s_Combined_CutEfficiency_Data_centralityClass_R0p%d_%d.pdf",Coll,radius,date.GetDate()),"RECREATE");
  ...
  hPP_CutEff->SetTitle(" ");
  hPP_CutEff->SetAxisRange(60, 300, "X");
  hPP_CutEff->SetAxisRange(0.8,1.1,"Y");
  hPP_CutEff->SetMarkerStyle(20);
  hPP_CutEff->Draw();

  cPP_CutEff->SaveAs(Form("../../Plots/PP_Combined_CutEfficiency_R0p%d_%d.pdf",radius, date.GetDate()));
  ...
  hPP_CutEff_Data->SetAxisRange(60, 300, "X");
  hPP_CutEff_Data->SetAxisRange(0.8,1.1,"Y");
  hPP_CutEff_Data->SetMarkerStyle(20);
  hPP_CutEff_Data->Draw();

  cPP_CutEff_Data->SaveAs(Form("../../Plots/PP_Combined_CutEfficiency_Data_R0p%d_%d.pdf",radius, date.GetDate()));
