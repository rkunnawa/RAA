...
void RAA_dataDrivenUnfoldingErrorCheck(int radius = 3, int radiusPP = 3, char* algo = (char*) "Pu", char *jet_type = (char*) "PF", int unfoldingCut = 40, char* etaWidth = (char*) "0_absEta_20", double deltaEta = 4.0){
...
//dPbPb_TrgComb[i] = (TH1F*)dPbPb_Trg80[i]->Clone(Form("Jet_80_triggered_spectra_data_PbPb_cent%d",i));
  ...
  dPP_1 = (TH1F*)fData_pp_in->Get(Form("hpp_Trg80_R%d_%s",radiusPP,etaWidth)); 
  dPP_1->Print("base");
  dPP_2 = (TH1F*)fData_pp_in->Get(Form("hpp_Trg60_R%d_%s",radiusPP,etaWidth));
  dPP_2->Print("base");
  dPP_3 = (TH1F*)fData_pp_in->Get(Form("hpp_Trg40_R%d_%s",radiusPP,etaWidth));
  dPP_3->Print("base");
  dPP_Comb = (TH1F*)fData_pp_in->Get(Form("hpp_TrgComb_R%d_%s",radiusPP,etaWidth));   
//dPP_Comb = (TH1F*)dPP_1->Clone(Form("hpp_TrgComb_R%d_n20_eta_p20",radiusPP,etaWidth));   
  ...
  dPbPb_TrgComb[i]->Scale(1./ dPbPb_TrgComb[i]->GetBinContent(nbins_pt));
  ...
  Doublt_t meanUnfoldPP[nbins_pt][unfoldingTrials], sigmaUnfoldPP[nbins_pt][unfoldingTrials]; 
  ...
  for(int u = 0;u<unfoldingTrials;++u){
    ...
    for(int i = 0;i<nbins_cent;++i){
      ...
      TH1F * hAfterUnfoldingSpectra = new TH1F("hAfterUnfoldingSpectra","",nbins_pt,boundaries_pt);
	...
	hPreUnfoldingSpectra->SetBinError(j+1, sigmaMeasPbPb[j][i]/sqrt(unfoldingTrials));
      ...
      removeZero(hMCGen);
      beysianUnfold myUnfoldingMulti(mPbPb_Matrix[i], hMCGen, 0);
    ...
    TH1F * hAfterUnfoldingSpectraPP = new TH1F("hAfterUnfoldingSpectraPP","",nbins_pt,boundaries_pt);

    for(int j = 0;j<nbins_pt;++j){
	
      hPreUnfoldingSpectraPP->SetBinContent(j+1, random->Gaus(meanMeasPP[j][i], sigmaMeasPP[j][i]));
      hPreUnfoldingSpectraPP->SetBinError(j+1, sigmaMeasPP[j][i]/sqrt(unfoldingTrials));
        
    }// nbins_pt loop
    TH1F* hMCGenPP          = (TH1F*)mPP_Response->ProjectionX();
    removeZero(hMCGenPP);
    beysianUnfold myUnfoldingMultiPP(mPP_Matrix, hMCGenPP, 0);
    ...
    for(int j = 0;j<nbins_pt;++j){
      ...
      meanUnfoldPP[j][i][u] = hAfterUnfoldingSpectraPP->GetBinContent(j+1);
      sigmaUnfoldPP[j][i][u] = hAfterUnfoldingSpectraPP->GetBinError(j+1);

    }// nbins_pt loop

    delete hPreUnfoldingSpectraPP;
    delete hAfterUnfoldignSpectraPP;
    delete hMCGenPP; 
  ...
  for(int i = 0;i<nbins_cent;++i){
    ...
    for(int j = 0;j<nbins_pt;++j){
      ...
      hAfterUnfoldingptBinDistribution[j] = new TH1F(Form("hAfterUnfoldingptBinDistribution_ptBin%d",j),"",1000,0,2);
      for(int u = 0;u<unfoldingTrials;++u){
  ...
  for(int j = 0;j<nbins_pt;++j){
    ...
    for(int u = 0;u<unfoldingTrials;++u){
      ...
      hAfterUnfoldingptBinDistribution[j]->Fill(j+1, meanUnfoldPP[j][u]);

    }// unfolding trials loop

    hCorrUnfoldingPP->SetBinContent(j+1, hAfterUnfoldingptBinDistribution[j]->GetMean());
    hCorrUnfoldingPP->SetBinError(j+1, hAfterUnfoldingptBinDistribution[j]->GetRMS());
