...
//_______________________________________________________________________________________________________________________
void doSVDUnfold(TString strInput = "/export/d00/scratch/rkunnawa/rootfiles/RAA/Pawan_TTree_PbPb_Data_MC_subid0_spectra_JetID_CutA_finebins_20_eta_20_R0p3.root", Int_t kregDraw = 3, Bool_t useToy = kTRUE) {
  ...
  Double_t nEvents = 2.196e7; // 21.96 M events 
  ...
  TH1D *fh1RawJetsCurrentUnfBinning = (TH1D*)fInput->Get("hpbpb_anaBin_HLTComb_R3_20_eta_20_cent0");
  fh1RawJetsCurrentUnfBinning->Print("base");
  ...
  TH2D *h2ResponseMatrix =  (TH2D*)fInput->Get("hpbpb_anaBin_matrix_HLT_R3_20_eta_20_cent0");
  h2ResponseMatrix->Print("base");
  ...
  Double_t ptmin = h2ResponseMatrix->GetYaxis()->GetXmin();
  Double_t ptmax = h2ResponseMatrix->GetYaxis()->GetXmax();

  //Smooth tail measured spectrum with power law fit -> used as measured prior
  TF1 *fPower = new TF1("fPower","[0]*TMath::Power(x,-([1]))",0.,400.);
  fh1RawJetsCurrentUnfBinning->Fit(fPower,"0","",60.,300.);
  ...
  hResponseMatrixTransposePrior->Print("base");
  ...
  cout<<"before getting the response matrix"<<endl;
  ...
  cout<<"after getting the response matrix"<<endl;
  ...
  for(Int_t ikreg=nKregMin; ikreg<nKregMax; ikreg++) {
    ...
    RooUnfoldSvd      unfoldSVD(&responseSVDMeas, fh1RawJetsCurrentUnfBinning, ikreg);
  ...
  TFile *histos = new TFile("SVDSpectra_test.root","RECREATE");
  hdi->Write("hdi");
  hSVal->Write("hSVal");
