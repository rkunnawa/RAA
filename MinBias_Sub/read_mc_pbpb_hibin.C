...
using namespace std;
//endfile was 494
void read_mc_pbpb_hibin(int startfile = 0, int endfile = 100, char *algo = "Pu", char *jet_type = "PF"){
  ...
  //infile1 = "doga_jet80_pbpb_data.txt";
    infile1 = "/net/hisrv0001/home/belt/wrk/JetRAA/PawanNtuple/MinBias/mc/PbPb_HydjetMinBias_newForest.txt";
  ...
  */
  ...
  TFile f(Form("PbPbMinBiasMC_data_vz_cent_ak_eventcount%s%s_%d_%d.root",algo,jet_type,date.GetDate(),endfile),"RECREATE");
  f.cd();
  ...
  TH1F *hEvents_MB = new TH1F("hEvents_HLTMB","",4,0,2);
  TH1F *hEvents = new TH1F("hEvents","",4,0,2);
  TH1F *hCentEvents = new TH1F("hCentEvents","",10,0,10);
  TH1F *hBinEvents= new TH1F("hBinEvents","",200,0,200);
  ...
  for(int k = 0;k<no_radius;k++){
	...
	for(int g = 0;g<nrefe_1;g++){
      ...
      if(jet80_1==1 && jetMB_1==1)hEvents_HLT80->Fill(1);
      if(jetMB_1==1)hEvents_MB->Fill(1);
