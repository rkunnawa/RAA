...
void RAA_calo_pf_JetCorrelation_v2(int startfile = 0, int endfile = 1, int radius=3, char *algo = "Pu", int deltaR=2/*which i will divide by 10 later when using*/, Float_t CALOPTCUT = 0.01, Float_t PFPTCUT = 0.01, char *dataset = "MC", char * etaWidth = "n20_eta_p20"){

  TH1::SetDefaultSumw2();
  TH1F *hpbpb_cent=new TH1F("hpbpb_cent","Centrality",200,0,200);
  TH1F *hpbpb_wcent=new TH1F("hpbpb_wcent","Centrality Weighted",200,0,200);
  TH1F *hpbpb_wzcent=new TH1F("hpbpb_wzcent","Centrality Weighted with Z",200,0,200);
  ...
  for(int k = 0;k<no_radius;k++){
    ...
    dir[2][k] = Form("ak%s%dCaloJetAnalyzer",algo,3);
    dir[3][k] = Form("ak%s%dPFJetAnalyzer",algo,radius);
  ...
  for(int k = 0;k<no_radius;k++){
    ...
    if(dataset=="MC"){
      ...
      // set the branch address for gen eta and phi here: 1 - Calo , 2- Pf for variables.
      //                     for Trees, 2- Calo, 3 - PF. refer Line 147
  ...
  for(Long64_t nentry = 0; nentry<1000;nentry++){
	...
	}// for filling MC closure histograms
#endif	  
	...
	//unmatchCaloJets->Fill();
      ...
      Float_t myweight_cent = hCentWeight->GetBinContent(hCentWeight->FindBin(hiBin));
      Float_t myweight_vz = fVz->Eval(vz);
      Float_t myfullweight=myweight_vz*myweight_cent;
      // cout<<" testing weights    "<< myweight_cent<<"      "<<myweight_vz<<endl;

      hpbpb_cent->Fill(hiBin); 
      hpbpb_wcent->Fill(hiBin,myweight_cent); 
      hpbpb_wzcent->Fill(hiBin,myfullweight); 
