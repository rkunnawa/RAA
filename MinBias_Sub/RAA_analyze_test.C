...
void RAA_analyze_test(int radius = 2, char* algo = (char*) "Pu", char *jet_type = (char*) "PF", int unfoldingCut = 40, char* etaWidth = (char*) "10_eta_18", double deltaEta = 1.6){
...
//= TFile::Open(Form("/afs/cern.ch/work/r/rkunnawa/WORK/RAA/CMSSW_5_3_18/src/Output/Pawan_ntuple_PbPb_data_MC_subid0_spectra_JetID_CutA_finebins_%s_R0p%d.root",etaWidth, radius)); 
  ...
  if(MCClosure) TFile * fMCClosure = TFile::Open("/export/d00/scratch/rkunnawa/rootfiles/RAA/Histogram_pp_PbPb_unfoldMatrix.root");
  
  TFile *  fMinBias = TFile::Open(Form("Pawan_ntuple_PbPb_MinBiasData_spectra_JetID_CutA_finebins_CentralityWeighted_%s_R0p%d.root",etaWidth,radius)); //MinBias File 
  ...
  for(int i = 0;i<nbins_cent;++i){
    ...
    if(dofakeremove){
      ...
      Float_t bin_end=dPbPb_TrgComb[i]->FindBin(20);
      
      Float_t   bin_nomb = hMinBias[i]->FindBin(15);
      Float_t bin_endmb=hMinBias[i]->FindBin(20);
  ...
  TFile fout(Form("Pawan_ntuple_PbPb_pp_calopfpt_ppNoJetidcut_R0p%d_noFakeWeight_unfold_mcclosure_oppside_trgMC_BothSmear_%s_%dGeVCut_ak%s_%d.root", radius, etaWidth, unfoldingCut, jet_type, date.GetDate()),"RECREATE");
  fout.cd();
  ...
  if(MCClosure){  
    ...
    for(int j = 1;j<=Iterations;j++){
      ...
      if(MCClosure){   uPbPb_MC_BayesianIter[i][j-1] = (TH1F*)uPbPb_MC_BayesianIter[i][j-1]->Rebin(nbins_pt,Form("uPbPb_MC_BayesianIter%d_cent%d",j,i),boundaries_pt);
  ...
  if(MCClosure){  uPP_MC_Bayes = (TH1F*)uPP_MC_Bayes->Rebin(nbins_pt,"PP_MC_Bayes_unfolded_spectra",boundaries_pt);
