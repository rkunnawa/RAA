...
Int_t run_1, evt_1, lumi_1;
  ...
  if(mat==1)  for(long nentry = 0; nentry < entries; ++nentry ){
	...
	if(pfpt_1>15 && calopt_1>15)	hcalopf_TrgObjComb[cBin]->Fill(calopt_1,pfpt_1,prescl);
  ...
  // data unmatched loop:
  long    entriesunmatched = Data_unmatched->GetEntries();
      //entries = 1000;
  //   cout<<"Unmatched Data ntuple "<<endl;
      if(unm==1)  for(long nentry = 0; nentry < entriesunmatched; ++nentry ){
	  
	  //if(nentry%100000 == 0) cout<<" nentry for the umatched with unmatched part = "<<nentry<<endl;
	  Data_unmatched->GetEntry(nentry);
	  if(jet80_1==0)continue;
	  cout<<" nentry for the umatched with unmatched part = "<<nentry<<endl;
	  Int_t cBin = findBin(hiBin_1);
	  if(cBin == -1 || cBin >= nbins_cent) continue;
	  Float_t dphi=deltaphi(etajet80,eta_1);
	  Float_t Sumcand = chSum_1 + phSum_1 + neSum_1 + muSum_1;
	  if(isSymm && TMath::Abs(eta_1) > (Float_t)etaHigh/10) continue;       
	  if(!isSymm && (TMath::Abs(eta_1) < (Float_t)etaLow/10 || TMath::Abs(eta_1) > (Float_t)etaHigh/10)) continue;   
	  if(jetMB_1==1&&jet80_1==1&&eMax_1/Sumcand < 0.05){
	    hdphi_TrgObj80[cBin]->Fill(dphi,prescl);
	    hpt_TrgObj80[cBin]->Fill(pfpt_1,prescl);
	    hdphipt_TrgObj80[cBin]->Fill(pfpt_1,dphi,prescl);
	  }

	}

 //Ading the second loop here for the unmatched 1st set of jets: 
      for(long nentry2 = 0; nentry2 < entries; ++nentry2 ){
	//	if(nentry2%100000 == 0) cout<<" nentry for the unmatched with matched part= "<<nentry2<<endl;
	Data_matched->GetEntry(nentry2);
	if(jet80_1==0 )continue;
	if(nentry2%100000 == 0)	cout<<" nentry for the unmatched with matched part= "<<nentry2<<endl;
  ...
  TFile fout(Form("Pawan_ntuple_PbPb_MinBiasData_spectra_JetID_CutA_finebins_CentralityWeightedAllMB_%s_R0p%d.root",etaWidth,radius),"RECREATE");
  ...
  for(int i = 0; i<nbins_cent;++i){
    ...
    hdphi_TrgObj80[i]->Write();
    hdphipt_TrgObj80[i]->Write();
    hpt_TrgObj80[i]->Write();

    hcalopf_TrgObjComb[i]->Write();
    hcalopf_TrgObj80[i]->Write();
