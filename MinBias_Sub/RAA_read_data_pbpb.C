...
void RAA_read_data_pbpb(int startfile = 8,
  ...
  for(int nEvt = 0; nEvt < nentries; ++ nEvt) {
    ...
    for(int jet = 0; jet<nref_F; ++jet){
      ...
      bool PFElecCut_2 = false;
      int jetLoc = 0;
      // get the information about the jets in the jetTree about their locaiton.
      for(int jet2 = 0; jet2<nref_jS; ++jet2){
	if(pt_F[jet] == pt_jS[jet2]) jetLoc = jet2;	   
      }
      
      if(fabs(eta_F[jet]) > 2) continue;
      //if(isPFElecCut_eS[jet] != 1) continue;
      if(isMiMatch_eS[jet]) continue;
      if(pt_F[jet] <15) continue;

      Float_t Sumcand = chSum_F[jet] + phSum_F[jet] + neSum_F[jet] + muSum_F[jet];

      // my old method of drawing the different cuts 
      if(isClMatch_eS[jet] == 1){
	if(calopt_F[index_eS[jet]]/pt_F[jet] > 0.5 && calopt_F[index_eS[jet]]/pt_F[jet] <= 0.85 && eMax_F[jet]/Sumcand < ((Float_t)18/7 *(Float_t)calopt_F[index_eS[jet]]/pt_F[jet] - (Float_t)9/7)) PFElecCut = true;
	if(calopt_F[index_eS[jet]]/pt_F[jet] > 0.85) PFElecCut = true;
	if(calopt_F[index_eS[jet]]/pt_F[jet] <= 0.5 && eMax_F[jet]/Sumcand < 0.05) PFElecCut = true;
      }
      if(isClMatch_eS[jet] == 0)
	if(eMax_F[jet]/Sumcand < 0.05 ) PFElecCut = true;

      // // testing Pawan's method
      // if(eMax_F[jet] / Sumcand < 0.05) PFElecCut_2 = true;       

      // if(isClMatch_eS[jet]) {
      // 	Float_t ePFSel  = (18./7.*calopt_F[index_eS[jet]] / pt_F[jet]) - 9./7.;
      // 	if(( calopt_F[index_eS[jet]]/ pt_F[jet]  > 0.50)
      // 	   && (calopt_F[index_eS[jet]] / pt_F[jet] <= 0.85)
      // 	   && (eMax_F[jet]/Sumcand < ePFSel)
      // 	   )PFElecCut_2 = true;
      // }

      //if(PFElecCut_2 != PFElecCut) cout<<"something is going wrong here"<<endl;
      ...
      if(printDebug)cout<<jet<<", hiForest pT = "<<pt_F[jet]<<", jetTree pT = "<<pt_jS[jetLoc]<<", electronCut = "<<isPFElecCut_eS[jetLoc]<<", Calo pT = "<<calopt_F[index_eS[jet]]<<", onFly flag calculation = "<<PFElecCut<<", eMax from hiForest = "<<eMax_F[jet]<<", eMax from jet Tree = "<<eMax_jS[jetLoc]<<endl;
	...
	hpbpb_TrgObj55[cBin]->Fill(pt_F[jet], jet55_p_F* wght);
	hpbpb_raw_TrgObj55[cBin]->Fill(rawpt_F[jet], jet55_p_F* wght);
	hpbpb_anaBin_TrgObj55[cBin]->Fill(pt_F[jet], jet55_p_F* wght);
	hpbpb_JEC_TrgObj55[cBin]->Fill(pt_F[jet] * (1. + 0.02/nbins_cent*(nbins_cent-cBin)), jet55_p_F* wght);
	hpbpb_Smear_TrgObj55[cBin]->Fill(pt_F[jet] + rnd.Gaus(0,1), jet55_p_F* wght);
      }

      if(jet65_F == 1 && jet80_F == 0){
	hpbpb_TrgObj65[cBin]->Fill(pt_F[jet], wght);
	hpbpb_raw_TrgObj65[cBin]->Fill(rawpt_F[jet], wght);
	hpbpb_anaBin_TrgObj65[cBin]->Fill(pt_F[jet], wght);
	hpbpb_JEC_TrgObj65[cBin]->Fill(pt_F[jet] * (1. + 0.02/nbins_cent*(nbins_cent-cBin)), wght);
	hpbpb_Smear_TrgObj65[cBin]->Fill(pt_F[jet] + rnd.Gaus(0,1), wght);
      }
      
      if(jet80_F == 1){
	hpbpb_TrgObj80[cBin]->Fill(pt_F[jet], wght);
	hpbpb_raw_TrgObj80[cBin]->Fill(rawpt_F[jet], wght);
	hpbpb_anaBin_TrgObj80[cBin]->Fill(pt_F[jet], wght);
	hpbpb_JEC_TrgObj80[cBin]->Fill(pt_F[jet] * (1. + 0.02/nbins_cent*(nbins_cent-cBin)), wght);
	hpbpb_Smear_TrgObj80[cBin]->Fill(pt_F[jet] + rnd.Gaus(0,1), wght);
      }
