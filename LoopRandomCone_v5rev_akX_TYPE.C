...
void LoopRandomCone_v5rev_akX_TYPE(double rad=3, int nMaxEv=10000, const char* TYPE="test", const char* COLLTYPE, const char* inFile="you_messed_up.root", const char* outFile="dontTrustMe.root"){
  TFile *FileA = TFile::Open(Form("%s",inFile));
  TFile* outf = new TFile(Form("%s",outFile),"recreate"); 
  ...
  cout<<"Incomming file : "<<inFile<<endl;
  ...
  TTree* ic      = (TTree*)FileA->Get("icPu5JetAnalyzer/t");
  ...
  TTree* hcal    = (TTree*)FileA->Get("hcalNoise/hcalNoise");
  if(COLLTYPE=="pA")   TTree* ttrack  = (TTree*)FileA->Get("ppTrack/trackTree");
  if(COLLTYPE=="PbPb") TTree* ttrack  = (TTree*)FileA->Get("mergedTrack/trackTree");
